...
// filter out all B.C. languages in use today
//SCRIPTS.filter(s => (s.year <= 0) && (s.living == true)).forEach(l => console.log(l.name));

// Map

/*let bcAliveLangs = (SCRIPTS.filter(s => (s.year <= 0) && (s.living == true))); 
console.log(bcAliveLangs.map(s => s.name));*/
